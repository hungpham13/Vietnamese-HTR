...
class Trainer():
    def __init__(self, config, pretrained=True, augmentor=ImgAugTransform()):
        ...
        self.optimizer = AdamW(self.model.parameters(), betas=(0.9, 0.98),
                               eps=1e-09)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(self.valid_gen):
                ...
                img, tgt_input, tgt_output, tgt_padding_mask = batch['img'], \
                                                               batch[
                                                                   'tgt_input'], \
                                                               batch[
                                                                   'tgt_output'], \
    ...
    def predict(self, sample=None):
        ...
        for batch in self.valid_gen:
            ...
            if self.beamsearch:
                translated_sentence = batch_translate_beam_search(batch['img'],
                                                                  self.model)
                prob = None
            else:
    ...
    def precision(self, sample=None):
        ...
        acc_full_seq = compute_accuracy(actual_sents, pred_sents,
                                        mode='full_sequence')
        acc_per_char = compute_accuracy(actual_sents, pred_sents,
                                        mode='per_char')
    ...
    def visualize_prediction(self, sample=16, errorcase=False, fontname='serif',
        ...
        for vis_idx in range(0, len(img_files)):
            ...
            plt.imshow(img)
            plt.title(
                'prob: {:.3f} - pred: {} - actual: {}'.format(prob, pred_sent,
                                                              actual_sent),
                loc='left', fontdict=fontdict)
            plt.axis('off')
    ...
    def data_gen(self, lmdb_path, data_root, annotation,
        ...
        dataset = OCRDataset(lmdb_path=lmdb_path,
                             ...
                             image_height=self.config['dataset'][
                                 'image_height'],
                             image_min_width=self.config['dataset'][
                                 'image_min_width'],
                             image_max_width=self.config['dataset'][
                                 'image_max_width'])
